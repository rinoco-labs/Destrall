  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 max-w-lg mx-auto px-1">
      {ASSISTANT_STARTER_PROMPTS.map(({ label, prompt }) => (
        ...
        >
          {label === "What can you do?" ? <AppLogo variant="mark" size="xs" className="shrink-0" /> : null}
          {label}
        </button>
      ))}
    </div>
...
export function AssistantEmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 text-center min-h-[min(420px,50vh)]">
      <div
        className="w-14 h-14 rounded-2xl border border-brand/30 bg-brand/10 flex items-center justify-center mb-5 p-2"
        style={{
          background:
